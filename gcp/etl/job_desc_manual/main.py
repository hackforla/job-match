...
def retrieve_job_id():
    ...
    from job_search.job_id_backfill
    """
...
# Set the number of concurrent requests to limit hits to Indeed webserver. Setting to 30 for running locally.
# Lots of lag time due to using a VPN
...
def job_desc():
    ...
    df_job_10k = split_dataframe_to_chunks(df_job_search, 10000)
    i = 0
    for df_job_i in df_job_10k:
        loop = asyncio.get_event_loop()
        future = asyncio.ensure_future(run_urls(df_job_i.job_id.to_list()))
        responses = loop.run_until_complete(future)
        # Split into batch sizes
        list_of_responses = list(divide_list(responses, 1000))
        for i_response in list_of_responses:
            ...
            i += 1
            print('Processed and Uploaded: ' +
                  str(i*1000) + ' Job Descriptions')
    return
